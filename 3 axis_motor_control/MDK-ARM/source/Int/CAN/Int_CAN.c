...
void Int_CAN_Init(void)
{
    ...
    can_filter_config.FilterIdHigh = (CAN_GATEWAY_ID << 5);
...
void Int_CAN_SendData(uint8_t *data, uint32_t len)
{
    ...
    {
        if (HAL_GetTick() - t0 > 5) // 5ms还没腾出邮箱就放弃这一帧
        {
            return; // 状态帧丢一帧无所谓，下次100ms
            /* code */
        }
    ...
    can_message_header.StdId = CAN_MOTOR_ID; // 标准ID
    can_message_header.IDE = CAN_ID_STD;     // 标准帧
    can_message_header.RTR = CAN_RTR_DATA;   // 数据帧

    // 将发送的数据放置到发送邮箱
    if (HAL_CAN_AddTxMessage(&hcan1, &can_message_header, data, &mailbox_num) != HAL_OK)
    {
...
void Int_CAN_ReceiveData(CAN_Message *message, uint32_t *len)
{
    ...
    // while (1)
    // {
    *len = HAL_CAN_GetRxFifoFillLevel(&hcan1, CAN_RX_FIFO0);
    if (*len == 0)
    {
        return;
    ...
    for (uint8_t i = 0; i < *len; i++)
    {
        ...
        // 将要接收的消息存到接收邮箱队列0中
        HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &can_message_header, message[i].data);
